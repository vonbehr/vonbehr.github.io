---
title: Lebenslauf
permalink: /lebenslauf/
---

# Florian von Behr

**Senior Technical Artist & CG Supervisor · Pipeline Engineering für Automotive CGI**

Bremen · florian@florianvonbehr.de · 0151 4645 4645

---

# Profil

CG Supervisor und Technical Artist mit über 15 Jahren Erfahrung in Produktvisualisierung und Pipeline Entwicklung für die Automobilindustrie. Langjährige Projekterfahrung mit Porsche, Mercedes-Benz, BMW, Audi, Volkswagen und Tesla sowie internationalen Herstellern wie NIO und Pininfarina.

Spezialisiert auf die Konzeption und Weiterentwicklung skalierbarer CGI-Produktionspipelines, Workflow-Automatisierung mit Python und die Einführung moderner Rendering- und Materialstandards (USD, MaterialX, OpenPBR). Verantwortete zuletzt die technische Migration ganzer Produktionsumgebungen — von der Ablösung des Renderers bis zum Aufbau von Cloud-Rendering-Infrastruktur - und verbindet dabei technisches Know How mit einem klaren Blick für die täglichen Produktionsabläufe.

Neben der technischen Verantwortung begleite ich die fachliche Weiterentwicklung von Teams, treibe Technologieeinführungen voran und fungiere als Schnittstelle zwischen Kreativ-, IT- und Projektteams.

---

# Kernkompetenzen

**Pipeline Engineering** — CGI-Produktionspipelines, Workflow-Design & Prozessoptimierung, Pipeline-Automatisierung, Asset-Management, Produktionssupport

**Softwareentwicklung** — Python, PySide/Qt, Maya API, MEL, Git

**Rendering & Look Development** — Chaos V-Ray, Chaos Vantage, AWS Deadline, Cloud Rendering

**USD & Realtime** — USD, MaterialX, OpenPBR, MDL, NVIDIA Omniverse, Unreal Engine

**Führung & Zusammenarbeit** — Fachliche Teamleitung, Mentoring, Onboarding neuer Mitarbeitender, Technische Beratung, Qualitätsmanagement

---

# Berufserfahrung

## CG Supervisor
**The Scope GmbH · Hamburg** — seit 10/2017

Verantwortlich für die technische Weiterentwicklung der gesamten CGI-Produktion mit Fokus auf Pipeline Engineering, Workflow-Optimierung und Automatisierung.

**Verantwortungsbereiche**
- Konzeption und Weiterentwicklung der Produktionspipeline
- Einführung technischer Standards
- Betreuung der Renderfarm
- Entwicklung interner Tools
- Workflow-Automatisierung
- Technischer Support
- Mentoring neuer Mitarbeitender
- Zusammenarbeit mit Fotografen und Art Directors

**Ausgewählte Erfolge**

*Pipeline-Migration* — Planung und Umsetzung der vollständigen Migration von Maxwell Render auf Chaos V-Ray inklusive Anpassung bestehender Produktionsabläufe.

*Cloud Rendering* — Einführung von AWS Deadline als skalierbare Rendering-Infrastruktur inklusive Integration in die bestehende Produktionspipeline.

*Pipeline Management* — Einführung und Weiterentwicklung von OpenPype bzw. Ayon zur Verwaltung von Assets, Versionen und Produktionsprozessen. Umstellung auf ACES Colormanagement.

*Python-Entwicklung* — Entwicklung zahlreicher Python-Werkzeuge zur Automatisierung wiederkehrender Produktionsaufgaben, darunter automatisierte Shader-Konvertierung, Materialzuweisung, Data Preparation, Render Setup, AOV-Erstellung, Light Selects, Render Layer und Qualitätskontrollen.

*USD-Workflows* — Konzeption und Umsetzung moderner Material-Workflows auf Basis von USD, MaterialX, OpenPBR und MDL für Omniverse, Vantage und Unreal Engine.

**Kunden:** Porsche, Mercedes-Benz, BMW, Audi, Volkswagen, Škoda, Renault, Tesla, Lincoln, NIO, Pininfarina, Viessmann, L'Oréal

---

## Senior 3D Artist
**The Scope GmbH · Hamburg** — 04/2013 – 09/2017

Verantwortlich für die Erstellung hochwertiger CGI-Inhalte mit Schwerpunkt Automotive-Visualisierung.

- Optimierung der Produktionspipeline
- Look Development
- Lighting
- Shading
- Rendering
- Technische Unterstützung des Teams

## 3D Artist
**The Scope GmbH · Hamburg** — 03/2011 – 03/2013

Erstellung hochwertiger Produktvisualisierungen für internationale Kunden.

- Modellaufbereitung
- Shading
- Rendering
- Compositing
- Qualitätskontrolle

## Freiberuflicher 3D Artist
**Selbstständig** — 05/2006 – 02/2011

Selbstständige Tätigkeit mit Schwerpunkt Architektur-, Produkt- und Werbevisualisierung.

**Leistungen:** Architekturvisualisierung, Produktvisualisierung, Animation, Werbegrafik, Kundenberatung, Projektkoordination

**Auftraggeber:** Architekturbüros, Werbeagenturen, Museen

**Ausgewählte Projekte**

*Realm Of Light – A Brief History of Life* — Full-CG Dome-Produktion. Verantwortlich für Modeling, Texturing, Rendering und Shot-Produktion.

*Technisch-Historisches Museum Peenemünde* — Konzeption und Visualisierung einer Nachnutzung des historischen Sauerstoffwerks.

---

# Ausbildung

**Diplom-Ingenieur Architektur** — Technische Universität Berlin · 1998 – 2006

**Ausbildung zum Bauzeichner** — Architekturbüro Schmidt + Kraft · 1996 – 1998

**Abitur** — Gymnasium an der Hamburger Straße · 1994

---

# Software-Kompetenzen

**Experte:** Autodesk Maya, Chaos V-Ray, Chaos Vantage, AWS Deadline

**Sehr gut:** Adobe Photoshop, Ayon

**Gut:** NVIDIA Omniverse, Adobe Substance 3D Painter, Adobe Substance 3D Designer

**Grundkenntnisse:** Unreal Engine, Blender, Autodesk 3ds Max, CityEngine

**Pipeline & Standards:** USD, MaterialX, OpenPBR, MDL, ACES Colormanagement, OCIO

**Programmierung:** Python, PySide/Qt, Git, MEL

**Betriebssysteme:** Windows, macOS, Linux

---

# Ehrenamt

**2. Vorsitzender** — RCB Radclub Bremen

Verantwortlich für Marketing, Website, Teambekleidung, Guide für Einsteiger.

Seit 2026 in Ausbildung zum DOSB-C-Trainer Leistungssport (Radsport).

---

# Sprachen
Deutsch — Muttersprache
Englisch — C1

# Interessen
Rennrad und Gravel, Fotografie, Kochen
